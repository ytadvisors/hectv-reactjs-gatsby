...
const fields = [
  ...
  [
    {
      name: 'date-time',
      component: 'date-time-adder',
      type: 'text',
      labels: {
        label: 'Please identify your preferred times.',
        time_label: 'HR:MIN',
...
class SchedulePaidProgramForm extends Component {
  constructor(props) {
    super(props);
  ...
  onSubmit() {
    const { values, stripe, callbackFunc, terms } = this.props;
    const valid_submit =
  ...
  render() {
    const { overlay_settings: { event_times, price } } = this.props;
    ...
    return (
      <section className="schedule-paid-program">
        <ReactContainer
          {...this.props}
          title={`Schedule your program for ${price}`}
...
SchedulePaidProgramForm.propTypes = {
...
export default connect(mapStateToProps)(injectStripe(SchedulePaidProgramForm));
